<section class="header-aboutUs m-navBar ">

    <div class="mask flex fd-column jc-center">
        <p class=" mt-5 p-2 text-center f-white f-4 f-bold ">About Us</p>
    </div>

</section>

<section class="mt--5 animated">
    
    <div class="history container  mb-2">

        <div class="containerHistory">

            <div class="text flex fd-column jc-center">
                <p class="ourHistory class mb-3 f-4 f-bold">Our History</p>
                <p class="mb-2">Lorem ipsum dolor sit amet consectetur, adipisicing elit. Esse impedit nisi nemo voluptates vel minima sunt, at officia soluta laborum. Natus fuga tempora libero ullam, quidem amet sit cumque aut.</p>
                <p class="mb-2">Lorem ipsum dolor sit amet consectetur, adipisicing elit. Esse impedit nisi nemo voluptates vel minima sunt, at officia soluta laborum. Natus fuga tempora libero ullam, quidem amet sit cumque aut.</p>
               
            </div>

            <div class="container-img grid-2-column-tablet flex-columm-phone-2">
                <div class="image1"></div>
                <div class="image2"></div>
                <div class="image3"></div>
            </div>

        </div>
    
    </div>

</section>

<main class="resume-ContactUs animated container mt-5 mb-5">
    <div class="text">
        <p class=" f-light mb-1 ">Country's Most Loved!</p>
        <p class="f-3 f-bold mb-1">Welcome</p>
        <p class="f-2 f-bold mb-1">We Are Locally Crafted Food & Wine Serving Since 1978</p>
        <p class=" mb-1">Lorem ipsum dolor sit, amet consectetur adipisicing elit. Officia placeat dolores consequuntur quisquam assumenda provident distinctio alias, repudiandae suscipit quis, rem fugit earum neque totam cumque quam unde similique libero?</p>
    </div>

    <div class="onlyImg"></div>

    <div class="hours">
        <div class="info mb-5">
            <p class="f-3 f-bold text-center">hours</p>
            <p>Monday - Saturday | 9AM - 1PM</p>
            <p>Saturday - Sunday | 9AM - 4AM</p>
        </div>
    </div>

</main>

<section class="ourChefs animated mt-5">

    <div class="container flex  fd-column ">
        <p class="f-4 f-bold f-white mt-2 text-center">Our Chefs</p>

        <div class="chefs mt-2">

            <div class="grid-3-column-0-tablet">
                <div class="chef1">

                    <div class="background-text">
                        <p class="text-center f-white mb-1">Name</p>
                        <p class="text-center f-white">Ocupation</p>
                    </div>

                </div>


                <div class="chef2">

                <div class="background-text">
                        <p class="text-center f-white mb-1">Name</p>
                        <p class="text-center f-white">Ocupation</p>
                </div>

                </div>

                <div class="chef3">

                    <div class="background-text">
                        <p class="text-center f-white mb-1">Name</p>
                        <p class="text-center f-white">Ocupation</p>
                    </div>

                </div>

            </div>

            <div class="grid-2-column-0-tablet">

                <div class="chef4">
                    
                    <div class="background-text">
                        <p class="text-center f-white mb-1">Name</p>
                        <p class="text-center f-white">Ocupation</p>
                    </div>

                </div>

                <div class="chef5">
                    
                    <div class="background-text">
                        <p class="text-center f-white mb-1">Name</p>
                        <p class="text-center f-white">Ocupation</p>
                    </div>

                </div>

            </div>



        </div>

    </div>

</section>

<section class="resume-aboutUs animated container mt-5-i mb-5">

    <p class="mb-2 text-center">Lorem ipsum dolor sit amet consectetur adipisicing elit. Optio esse vel placeat, voluptatem quod reiciendis numquam pariatur labore sequi dolor assumenda? Voluptates officia quos minus quibusdam sed eligendi excepturi quaerat!</p>

    <div class="capacity container mw-800 mt-5">

        <div class="containerCapacity">

            <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-thumb-up" width="44" height="44" viewBox="0 0 24 24" stroke-width="1.5" stroke="black" fill="none" stroke-linecap="round" stroke-linejoin="round">
                <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                <path d="M7 11v8a1 1 0 0 1 -1 1h-2a1 1 0 0 1 -1 -1v-7a1 1 0 0 1 1 -1h3a4 4 0 0 0 4 -4v-1a2 2 0 0 1 4 0v5h3a2 2 0 0 1 2 2l-1 5a2 3 0 0 1 -2 2h-7a3 3 0 0 1 -3 -3" />
            </svg>

            <p class="f-3 f-bold">1550</p>
            <p class="f-2">Satisfied Guests</p>

        </div>

        <div class="containerCapacity">

            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -21 512 512">
                <path d="m490.152344 240.304688h-17.773438c-1.664062-27.023438-8.246094-53.105469-19.636718-77.625-1.738282-3.753907-6.191407-5.378907-9.945313-3.640626-3.753906 1.742188-5.382813 6.195313-3.640625 9.949219 10.464844 22.535157 16.570312 46.492188 18.210938 71.316407h-343.070313c-4.136719 0-7.488281 3.351562-7.488281 7.488281s3.351562 7.488281 7.488281 7.488281h375.855469c3.789062 0 6.867187 3.082031 6.867187 6.871094v9.140625c0 3.789062-3.078125 6.871093-6.867187 6.871093h-468.304688c-3.789062 0-6.867187-3.082031-6.867187-6.871093v-9.140625c0-3.789063 3.078125-6.871094 6.867187-6.871094h57.539063c4.136719 0 7.492187-3.351562 7.492187-7.488281s-3.355468-7.488281-7.492187-7.488281h-24.753907c3.300782-49.753907 24.636719-96.039063 60.597657-131.046876 34.65625-33.734374 79.4375-53.59375 127.296875-56.742187.027344 0 .054687.003906.082031 0 2.039063-.132813 4.054687-.234375 6.046875-.308594.398438-.011719.792969-.03125 1.191406-.042969.710938-.023437 1.417969-.039062 2.117188-.054687 1.34375-.023437 2.6875-.046875 4.035156-.046875 1.789062 0 3.578125.035156 5.363281.082031.527344.015625 1.050781.03125 1.582031.046875 1.375.046875 2.746094.117188 4.117188.191406.726562.039063 1.453125.078126 2.183594.125 61.488281 4.039063 118.058594 36.015626 153.261718 87.289063 2.339844 3.40625 7.003907 4.277344 10.414063 1.933594 3.410156-2.339844 4.277344-7.003907 1.933594-10.414063-34.640625-50.457031-88.558594-83.503906-148.152344-92.027344 3.734375-5.796874 5.832031-12.621093 5.832031-19.769531 0-4.070312-1.816406-11.574219-13.980468-16.050781-6.074219-2.234375-14.085938-3.46875-22.554688-3.46875-8.472656 0-16.484375 1.234375-22.558594 3.46875-12.164062 4.476562-13.976562 11.980469-13.976562 16.050781 0 7.128907 2.089844 13.941407 5.800781 19.726563-45.246094 6.402344-87.210937 26.886718-120.484375 59.277344-38.871094 37.839843-61.835938 87.949218-65.160156 141.777343h-17.773438c-12.046875.003907-21.847656 9.804688-21.847656 21.851563v9.140625c0 12.050781 9.800781 21.851562 21.847656 21.851562h80.683594c-4.027344 2.757813-7.886719 5.851563-11.5 9.285157l-.148438.140624c-.230468.214844-.449218.445313-.648437.691407l-35.558594 43.179687c-4.460937-2.832031-9.742187-3.863281-14.953125-2.863281-5.433594 1.039063-10.132812 4.132813-13.234375 8.710937l-13.984375 20.621094c-6.394531 9.433594-3.925781 22.320313 5.5 28.722656l95.691406 65.007813c3.46875 2.355469 7.476563 3.582031 11.582032 3.582031 1.304687 0 2.621094-.125 3.933594-.375 5.429687-1.039062 10.128906-4.132812 13.230468-8.710937l13.984375-20.621094c6.394531-9.433594 3.929688-22.320313-5.496093-28.722656l-.546876-.371094c6.660157-7.558594 15.539063-12.976563 25.199219-15.210937 20.355469-4.714844 44.308594-1.679688 60.554688 1.011718 20.097656 3.328125 41.207031 2.191406 61.046875-3.289062l27.578125-7.621094c18.371093-5.078125 35.277343-13.628906 50.253906-25.417969.113281-.089843.222656-.179687.332031-.277343l47.347656-41.933594c8.070313-6.175782 9.828126-16.871094 7.042969-25.539063h60.414063c12.046875 0 21.847656-9.800781 21.847656-21.847656v-9.144531c0-12.046875-9.800781-21.847656-21.847656-21.847656zm-234.152344-225.324219c12.613281 0 20.171875 3.121093 21.554688 4.855469-.101563 7.105468-3.695313 13.660156-9.542969 17.589843-1.472657-.078125-2.925781-.140625-4.375-.195312-.488281-.015625-.976563-.039063-1.464844-.050781-2.089844-.0625-4.152344-.09375-6.171875-.09375-1.433594 0-2.894531.019531-4.367188.050781-.710937.011719-1.425781.03125-2.140624.054687-.851563.027344-1.710938.054688-2.570313.089844-.980469.042969-1.960937.085938-2.9375.140625-5.847656-3.929687-9.4375-10.484375-9.539063-17.585937 1.382813-1.734376 8.941407-4.855469 21.554688-4.855469zm53.710938 281.777343c-.558594-1.265624-1.226563-2.46875-1.980469-3.613281h71.839843l-31.0625 20.558594-44.277343 12.808594-1.503907.035156c8.523438-7.105469 11.632813-19.238281 6.984376-29.789063zm-81.21875-3.613281-10.808594 2.253907-8.355469-2.253907zm-84.464844 138.964844-13.980469 20.621094c-.855469 1.265625-2.152344 2.117187-3.652344 2.40625-1.496093.285156-3.019531-.027344-4.28125-.886719l-95.691406-65.007812c-2.601563-1.765626-3.28125-5.320313-1.515625-7.921876l13.984375-20.625c.855469-1.261718 2.148437-2.117187 3.648437-2.402343.363282-.070313.726563-.105469 1.085938-.105469 1.132812 0 2.238281.339844 3.195312.992188l5.167969 3.507812c0 .003906.003907.003906.003907.003906l90.519531 61.492188c2.601562 1.765625 3.28125 5.324218 1.515625 7.925781zm269.476562-125.253906c-.179687.128906-.351562.269531-.519531.417969l-47.398437 41.984374c-13.363282 10.480469-28.441407 18.089844-44.8125 22.613282l-27.578126 7.621094c-17.757812 4.910156-36.640624 5.929687-54.605468 2.953124-17.507813-2.898437-43.429688-6.144531-66.386719-.828124-13.296875 3.078124-25.4375 10.71875-34.269531 21.34375l-70.792969-48.089844 34.359375-41.726563c12.996094-12.269531 29.476562-19.554687 45.257812-20h.304688c4.894531-.082031 10.074219 1.359375 15.554688 2.894531 1.101562.304688 2.199218.613282 3.300781.910157l49.59375 13.378906c1.136719.304687 2.328125.339844 3.480469.097656l64.953124-13.542969c2.945313-.609374 6.039063-.011718 8.488282 1.648438 1.582031 1.074219 2.816406 2.550781 3.570312 4.265625 2.226563 5.050781.128906 10.980469-4.847656 13.542969l-22.160156 11.734375c-3.066406 1.625-4.613282 5.140625-3.742188 8.5.875 3.363281 3.933594 5.652343 7.410156 5.605469l32.871094-.710938c.652344-.015625 1.296875-.113281 1.917969-.292969l46.316406-13.398437c.730469-.210938 1.421875-.53125 2.054688-.949219l46.726562-30.929687c3.921875-2.421876 9.140625-4.414063 11.917969-.316407 2.164062 3.1875 2.214844 8.925781-.964844 11.273438zm0 0"/>
            </svg>

            <p class="f-3 f-bold">125</p>
            <p class="f-2">Launches Every Day</p>

        </div>

        <div class="containerCapacity">

        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Capa_1" x="0px" y="0px" viewBox="0 0 480 480" style="enable-background:new 0 0 480 480;" xml:space="preserve">
            <g>
                <g>
                    <g>
                        <path d="M472,112c-4.418,0-8,3.582-8,8v168h-88v-72c0-13.255-10.745-24-24-24h-8v-25.136c14.086-3.637,23.945-16.317,24-30.864     V88h-64v48c0.055,14.547,9.915,27.227,24,30.864V192h-40v-8c0-13.255-10.745-24-24-24h-16v-56h8c22.08-0.026,39.974-17.92,40-40     V40h-16v24c0,13.255-10.745,24-24,24h-8V40h-16v48h-8c-13.255,0-24-10.745-24-24V40h-16v24c0.026,22.08,17.92,39.974,40,40h8v56     h-16c-13.255,0-24,10.745-24,24v8h-40v-25.136c14.086-3.637,23.945-16.317,24-30.864V88h-64v48     c0.055,14.547,9.915,27.227,24,30.864V192h-8c-13.255,0-24,10.745-24,24v72H16V120c0-4.418-3.582-8-8-8s-8,3.582-8,8v344     c0,4.418,3.582,8,8,8s8-3.582,8-8v-40h88v40c0,4.418,3.582,8,8,8s8-3.582,8-8v-20.192L144.376,424H152v40c0,4.418,3.582,8,8,8     s8-3.582,8-8v-59.2l72-58.504l72,58.504V464c0,4.418,3.582,8,8,8s8-3.582,8-8v-40h7.624L360,443.808V464c0,4.418,3.582,8,8,8     s8-3.582,8-8v-40h88v40c0,4.418,3.582,8,8,8s8-3.582,8-8V120C480,115.582,476.418,112,472,112z M104,408H16v-40h88V408z M104,352     H16v-16h88V352z M104,320H16v-16h88V320z M320,104h32v16h-32V104z M320,136h32c0,8.837-7.163,16-16,16S320,144.837,320,136z      M208,184c0-4.418,3.582-8,8-8h48c4.418,0,8,3.582,8,8v8h-64V184z M128,104h32v16h-32V104z M128,136h32c0,8.837-7.163,16-16,16     S128,144.837,128,136z M120,423.2V248.8L227.312,336L120,423.2z M134.528,240h210.944L240,325.696L134.528,240z M360,288v135.2     L252.688,336L360,248.8V288z M360,224H120v-8c0-4.418,3.582-8,8-8h224c4.418,0,8,3.582,8,8V224z M464,408h-88v-40h88V408z      M464,352h-88v-16h88V352z M464,320h-88v-16h88V320z"/>
                        <rect x="184" y="8" width="16" height="16"/>
                        <rect x="232" y="8" width="16" height="16"/>
                        <rect x="280" y="8" width="16" height="16"/>
                    </g>
                </g>
            </g>

        </svg>

            <p class="f-3 f-bold">35</p>
            <p class="f-2">Family Tables</p>

        </div>

        <div class="containerCapacity">

        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Layer_1" x="0px" y="0px" viewBox="0 0 469.333 469.333" style="enable-background:new 0 0 469.333 469.333;" xml:space="preserve">
			<path d="M128,224c0-5.896-4.771-10.667-10.667-10.667c-5.896,0-10.667,4.771-10.667,10.667c0,28.01,30.896,66.469,34.417,70.771     c2.115,2.563,5.177,3.896,8.26,3.896c2.375,0,4.771-0.792,6.76-2.417c4.552-3.74,5.208-10.458,1.479-15.021     C146.146,267.292,128,239.437,128,224z"/>
			<path d="M352,213.333c-5.896,0-10.667,4.771-10.667,10.667c0,15.385-18.146,43.271-29.583,57.24     c-3.729,4.552-3.063,11.271,1.49,15.01c1.979,1.625,4.375,2.417,6.76,2.417c3.083,0,6.135-1.333,8.25-3.896     c3.521-4.302,34.417-42.76,34.417-70.771C362.667,218.104,357.896,213.333,352,213.333z"/>
			<path d="M234.667,213.333c-5.896,0-10.667,4.771-10.667,10.667v64c0,5.896,4.771,10.667,10.667,10.667     s10.667-4.771,10.667-10.667v-64C245.333,218.104,240.562,213.333,234.667,213.333z"/>
			<path d="M352,42.667c-6.464,0-13.219,0.855-20.234,2.158C308.273,17.439,273.496,0,234.667,0s-73.607,17.439-97.099,44.824     c-7.016-1.302-13.771-2.158-20.234-2.158C52.635,42.667,0,95.302,0,160c0,44.333,25,84.635,64.25,104.562L64.042,425.76     c-0.01,11.635,4.51,22.573,12.729,30.802c8.229,8.24,19.156,12.771,30.792,12.771h254.25c24,0,43.521-19.521,43.521-43.521     V264.406c39.104-19.979,64-60.219,64-104.406C469.333,95.302,416.698,42.667,352,42.667z M384,425.813     c0,12.229-9.958,22.188-22.188,22.188h-254.25c-5.927,0-11.5-2.313-15.698-6.51c-4.188-4.198-6.5-9.771-6.49-15.708L85.43,384     H384V425.813z M390.604,247.802c-0.563,0.229-1.094,0.5-1.594,0.823c-1.344,0.833-2.448,1.938-3.271,3.208     c-0.833,1.26-1.396,2.719-1.625,4.281c-0.094,0.594-0.125,1.188-0.115,1.792v104.76H85.457l0.137-104.615     c0.01-0.583-0.021-1.177-0.104-1.75c-0.229-1.615-0.823-3.115-1.688-4.417c-0.813-1.219-1.875-2.281-3.167-3.094     c-0.531-0.333-1.094-0.625-1.677-0.865C43.948,232.615,21.333,198.135,21.333,160c0-52.938,43.063-96,96-96     c2.069,0,4.253,0.25,6.396,0.419c-10.792,18.756-17.063,40.43-17.063,63.581c0,5.896,4.771,10.667,10.667,10.667     c5.896,0,10.667-4.771,10.667-10.667c0-58.813,47.854-106.667,106.667-106.667S341.333,69.187,341.333,128     c0,5.896,4.771,10.667,10.667,10.667c5.896,0,10.667-4.771,10.667-10.667c0-23.151-6.271-44.824-17.063-63.581     C347.747,64.25,349.931,64,352,64c52.938,0,96,43.063,96,96C448,198,425.479,232.437,390.604,247.802z"/>
        </svg>

            <p class="f-3 f-bold">18</p>
            <p class="f-2">Team Members</p>

        </div>

    </div>
    
</section>

<section class="restaurantServices animated mb-5">

    <div class="mt-5 container ">

        <h2 class="mb-4 text-center">Restaurant Amenities</h2>

        <div class="capacity  container mw-800 ">

            <div class="service flex fd-column ai-center ">
                <img class="s-5 invert-bw-svg" src="/build/img/wifi.svg" alt="wifi">
                <p>Free Wifi</p>
            </div>

            <div class="service flex fd-column ai-center ">
                <img class="s-5 invert-bw-svg" src="/build/img/smokingZone.svg" alt="smoking">
                <p>Smoking Zone</p>
            </div>

            <div class="service flex fd-column ai-center ">
                <img class="s-5 invert-bw-svg" src="/build/img/music.svg" alt="music">
                <p>Live Music</p>
            </div>

            <div class="service flex fd-column ai-center ">
                <img class="s-5 invert-bw-svg" src="/build/img/parking.svg" alt="parking">
                <p>Parking</p>
            </div>

        </div>



    </div>

</section>

<section class="testimonials animated ds mt-5 mb-5 ">
    <p class="text-center f-3 f-light mb-5">Happy customers!</p>

    <div class="container mw-800 ">
        <!-- Slider main container -->
        <div class="swiper-container">
        <!-- Additional required wrapper -->
            <div class="swiper-wrapper">
                <!-- Slides -->
                <div class="swiper-slide">
                    <div class="testimonial">
                        <div class="profilePhoto">
                            <div class="img img1"></div>
                        </div>
                        <p class="f-light mt-2">"Lorem, ipsum dolor sit amet consectetur adipisicing elit. Laborum obcaecati animi optio modi ullam saepe! Repellat, labore eaque consequuntur facilis nisi blanditiis enim earum quaerat molestiae doloremque, possimus ut voluptatibus"</p>
                        <P class="f-light mt-2">- Full Name</P>
                    </div>
                </div>
                <div class="swiper-slide">                 
                    <div class="testimonial">
                        <div class="profilePhoto">
                            <div class="img img2"></div>
                        </div>
                        <p class="f-light mt-2">"Lorem, ipsum dolor sit amet consectetur adipisicing elit. Laborum obcaecati animi optio modi ullam saepe! Repellat, labore eaque consequuntur facilis nisi blanditiis enim earum quaerat molestiae doloremque, possimus ut voluptatibus"</p>
                        <P class="f-light mt-2">- Full Name</P>
                    </div>
                </div>

                <div class="swiper-slide">
                    <div class="testimonial">
                        <div class="profilePhoto">
                            <div class="img img3"></div>
                        </div>
                        <p class="f-light mt-2">"Lorem, ipsum dolor sit amet consectetur adipisicing elit. Laborum obcaecati animi optio modi ullam saepe! Repellat, labore eaque consequuntur facilis nisi blanditiis enim earum quaerat molestiae doloremque, possimus ut voluptatibus"</p>
                        <P class="f-light mt-2">- Full Name</P>
                    </div>
                </div>
                ...
            </div>

            <!-- If we need navigation buttons -->
            <div class="swiper-button-prev blue_important"></div>
            <div class="swiper-button-next blue_important"></div>

        </div>
    </div>

</section>  

<section class="invite animated mt-5 mb-5 ">

    <div class="">
        <p class="text-center f-3 f-lights mb-2">Have a nice night</p>
        <img class="mw-800 ds container" src="/build/img/undraw_special_event_4aj8.svg" alt="">
    </div>


    <div class="checkout  flower-opacity mt-5 container mw-800">
    
        <div class="antifilter flex fd-column jc-center ai-center">
            <div class="center-all ">
                <p class="text-center f-ligths mt-5 mb-2">Join Us</p>
                <p class="text-center f-3 f-lights mb-2">Book A Table Now, Come And See Us</p>
                <a href="/reservation" class="linkReservation inline-block button-default  mt-2 ">
                    <div>
                        <p class="f-2 f-normal uppercase">reservation</p>
                    </div>
                </a>
            </div>

            <p class="text-center  mt-2">Checkoout Out Our Reviews</p>

            <div class="social mt-2 " >
                <img class="pointer ds" src="../../build/img/facebook.svg" alt="facebook-icon">
                <img class="pointer ds" src="../../build/img/instagram (1).svg" alt="instagram-icon">
                <img class="pointer ds" src="../../build/img/twitter.svg" alt="twitter-icon">
            </div>
        </div>
 

    </div>


</section>